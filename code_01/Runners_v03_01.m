...
for iter=1:length(selected)
    ...
    cd(DataFolder)
    fname=dir('*runme*');    
    xlswrite(fname.name,[datar(x,MC),datar(x,MCT)]',SheetName,'A1:A2'),
    xlswrite(fname.name,[datar(x,Kr),datar(x,Mr),datar(x,Nr)]',SheetName,'A3:A5'),    
    xlswrite(fname.name,[datar(x,Rr),datar(x,TxdB),datar(x,RdB)]',SheetName,'A7:A9'),    
    cd(CodeFolder)
