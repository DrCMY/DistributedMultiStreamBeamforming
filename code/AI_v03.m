function AI_v03(MonteCarlo,MonteCarloT,K,M,N,d,R,B,PdBTxRx,PdBRRx,sigma2k1,sigma2k2,sigma2r,LPPdB,iterlimit,rhoF,rhocF,rhoU,rhocU,Delta,PDelta,Name1,Name2,faultlimit,wmin,wstep,wmax,InputFolder)
tgGlobal=tic;
...
[fi1,fi2,fi3,fi4,fi5,fi6,fido1,fido2,fido2E,fido3,fido3E,fido4,fido5,fido5E,fido6,fido6E,fido7,fido7E,fido11,fido11E,fido14,fido14E,Name]=IOFiles_v02(Name1,Name2,cverSubFile_b);
ExplanatoryFiles_UFIterII_v01(fido2E,fido3E,fido5E,fido6E,fido7E,fido11E,fido14E,fido1,Name2,Delta,iterlimit,rhoF,rhocF,rhoU,rhocU,faultlimit,wmin,wstep,wmax)
...
while mc-missed(1)<=MonteCarloT && mc<=MonteCarlo % Due to the missed channels MonteCarlo variable is set to be a higher value than the MonteCarloT (Monte Carlo target) variable. For instance, if you want to test 20 Monte Carlo channels, you should set MonteCarloT=20 and a safe MonteCarlo>MonteCarloT value, such as MonteCarlo=30. Therefore, in case, 10 channels are missed, 20 channels can still be tested
        ...
        [F,hYr]=F_hYrM_v01(K,R,M,N,d,Hpp,U,F,PLinRRx,sigma2r); % Rescaling the relay filter powers to their maximum power values. hYrM matrix stores the \mathbf{Y}_r matrices in (43)
        [Xkl,sinr]=XklM_sinrx_v01(K,R,M,N,B,d,J,Hpp,G,U,F,V,sigma2k1,sigma2k2,sigma2r); % Calling a subfunction to obtain the initial SINR values of each stream (sinrx variable) and covariances of transmit beamforming vectors (XklMp)
        ...
        while sum(checkwhilei)<K && iter<=iterlimit              % Algorithm is terminated either when all SINR targets are achieved or maximum number of iterations is achieved 
            ...
            [Rdk,Rbkl,sigma2kl]=RdkM_RbklM_sigma2kl_UFIterII_v01(K,R,M,N,B,d,J,Hpp,G,F,V,sigma2k1,sigma2k2,sigma2r); % See the subfunction for the details
            [U,Xkl]=U_XklM_UFIterII_v03(K,M,d,Rdk,Rbkl,sinrt,sigma2kl,U,Uopt,Xkl,Xklopt,zetavU,iter,Delta,PDelta,PLinTxRx);      % See the subfunction for the details
            ...
            [Deltax,iter,counter,checkwhilei,missed,fault,DeltaLogs,AverageDeltaLogs]=Delta_Logging_v01(K,sinr,sinrt,Delta,DeltaLogs,AverageDeltaLogs,counter,iter,steprange,missed,fault,faultlimit,iterlimit,wmin,mc,fido6,fido11); % See the subfunction for the details           
            ...
            Drkl=DrklM_v01(K,R,M,N,B,d,Hpp,U,F,sigma2r); 
            zetavpF=zetavF;     % Previous values are stored
            zetanvpF=zetanvF;   % Previous values are stored         
            [F,hYr]=F_hYrM_UFIterII_v02(K,N,d,R,sinrt,sigma2r,N2,N2p1,Drkl,tCklrin,zetavF,zetanvF,PLinRRx,Delta,PDelta,iter,F,Fopt,hYr,hYropt); % Relay beamforming F and covariance hYrM matrices are updated
        ...
        [PTot,PTx,PR]=PTot_PTx_PR_UFIterII_v01(K,R,M,N,d,Hpp,Xkl,Rdk,U,Fopt,sigma2r); % Evaluating total power consumption (PTot) and power consumption of transmitters (PTx)
        [sinr_MC,P_MC,missed,iter_Logger,iter_MC]=Logging_v03(K,R,B,PTot,PTx,PR,P_MC,PLinTxRx,PLinRRx,sinr_MC,sinrt,sinr,checkwhilei,iter,iter_MC,PDelta,DeltaLogs,missed,mc,fault,faultlimit,iter_Logger,fido1,fido2,fido4,fido5,fido7,fido11,fido14);        
