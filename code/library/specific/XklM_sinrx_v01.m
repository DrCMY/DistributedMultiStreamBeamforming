function [XklM,sinrx]=XklM_sinrx_v01(K,R,M,N,B,d,J,HppM,G,U,F,V,sigma2k1,sigma2k2,sigma2r)
sinrx=zeros(K,d);
XklM=zeros(M,B*M);
    ...
    for l=1:d
        ...
        for i=0:K-1
            ...
            for r=0:R-1 % number of relays in each cluster
                rangerN=r*N+1:r*N+N;
                Hpki=Hpki+G(rangekM,rangerN)*F(rangerN,:)*HppM(rangerN,rangeiM);
            end
        ...
        XklM(:,rangeklM)=U(:,pointkl)*U(:,pointkl)';
    end
end
sinrx=real(sinrx);
